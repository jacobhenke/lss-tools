...
/**
 * Lss_Tools Sanitization Helper Class
 *
 ...
 */
class Lss_Tools_Sanitization_Helper {
	...
	public function __construct( $plugin_name ) {
		...
		add_filter( 'lss_tools_settings_sanitize_text', array( $this, 'sanitize_text_field' ) );
		add_filter( 'lss_tools_settings_sanitize_email', array( $this, 'sanitize_email_field' ) );
		add_filter( 'lss_tools_settings_sanitize_checkbox', array( $this, 'sanitize_checkbox_field' ) );
		add_filter( 'lss_tools_settings_sanitize_url', array( $this, 'sanitize_url_field' ) );
	}
	...
	private function apply_type_filter( $input, $tab, $key ) {
		...
		return apply_filters( 'lss_tools_settings_sanitize_' . $type, $input[$key], $key );
	}

	private function apply_general_filter( $input, $key ) {

		return apply_filters( 'lss_tools_settings_sanitize', $input[$key], $key );
	}

	// Key specific on change hook
	private function do_settings_on_key_change_hook( $key, $new_value ) {

		$old_plugin_settings = get_option( 'lss_tools_settings' );
		...
		if ( isset($old_plugin_settings[$key]) && $old_plugin_settings[$key] !== $new_value ) {
			...
			do_action( 'lss_tools_settings_on_change_' . $key, $new_value, $old_plugin_settings[$key] );
	...
	private function do_settings_on_change_hook( $new_values, $tab ) {
		...
		if ( $changed ) {
			...
			do_action( 'lss_tools_settings_on_change_' . $tab, $new_values, $old_plugin_settings );
