...
/**
 * Lss_Tools Callback Helper Class
 *
 ...
 */
class Lss_Tools_Callback_Helper {
	...
	private function get_id_attribute( $id ) {
		return ' id="lss_tools_settings[' . $id . ']" ';
	}

	private function get_name_attribute( $name ) {
		return ' name="lss_tools_settings[' . $name . ']" ';
	}
	...
	private function get_label_for( $id, $desc ) {
		return '<label for="lss_tools_settings[' . $id . ']"> '  . $desc . '</label>';
	}
	...
	public function checkbox_callback( $args ) {
		...
		$value = Lss_Tools_Option::get_option( $args['id'] );
	...
	public function multicheck_callback( $args ) {
		...
		$old_values = Lss_Tools_Option::get_option( $args['id'], array() );
	...
	public function radio_callback( $args ) {
		...
		$old_value = Lss_Tools_Option::get_option( $args['id'] );
	...
	private function input_type_callback( $type, $args ) {
		...
		$value = Lss_Tools_Option::get_option( $args['id'], $args['std']  );
	...
	public function rich_editor_callback( $args ) {
		...
		if ( $wp_version >= 3.3 && function_exists( 'wp_editor' ) ) {
			ob_start();
			wp_editor( stripslashes( $value ), 'lss_tools_settings_' . $args['id'], array( 'textarea_name' => 'lss_tools_settings[' . $args['id'] . ']' ) );
			$html = ob_get_clean();
		} else {
	...
	public function upload_callback( $args ) {
		...
		$html .= $this->get_id_and_name_attrubutes( $args['id'] );
		$html .= 'class="' . $args['size'] . '-text ' . 'lss_tools_upload_field" ';
		$html .= ' value="' . esc_attr( stripslashes( $value ) ) . '"/>';

		$html .= '<span>&nbsp;<input type="button" class="' .  'lss_tools_settings_upload_button button-secondary" value="' . __( 'Upload File', $this->plugin_name ) . '"/></span>';
